...
export async function ensureDefaultAdminExists() {
    const defaultUsername = 'admin@trendplus.ly';
...
export async function getAvailableCards(): Promise<Card[]> {
    try {
        const q = query(
...
export async function addCard(card: Omit<Card, 'id'>): Promise<Card | null> {
    try {
        const docRef = await addDoc(collection(db, CARDS_COLLECTION), card);
...
export async function updateCard(cardId: string, data: Partial<Card>): Promise<boolean> {
    try {
...
export async function purchaseCard(cardId: string, userId: string, userName: string): Promise<{ success: boolean; message: string; card?: Card }> {
    try {
        return await runTransaction(db, async (transaction) => {
            const cardRef = doc(db, CARDS_COLLECTION, cardId);
            const userRef = doc(db, USERS_COLLECTION, userId);
            ...
            // Add Transaction Record (Debt)
            const transactionRef = doc(collection(db, TRANSACTIONS_COLLECTION));
            await transaction.set(transactionRef, {
                userId: userId, // Legacy field if needed, but we use customerId usually
                customerId: userId,
                customerName: userName,
                date: new Date().toISOString(),
                type: 'order', // Treat as order/purchase
                status: 'completed',
                amount: cardData.price,
                description: `شراء بطاقة: ${cardData.service} - ${cardData.name}`
            });

            // Update User Debt
            // We assume 'debt' is the field to increment.
            // Note: RecalculateUserStats usually fixes this, but we can do an optimistic increment here or let the stats run.
            // Let's increment directly.
            const currentDebt = userDoc.data().debt || 0;
            transaction.update(userRef, {
                debt: currentDebt + cardData.price,
                orderCount: (userDoc.data().orderCount || 0) + 1 // Treat as an "order" or keep separate? Let's just update debt.
            });

            return { success: true, message: "تم شراء البطاقة بنجاح", card: { ...cardData, status: 'sold', soldToUserId: userId, soldAt: new Date().toISOString() } };
        });
    } catch (error: any) {
        console.error("Error purchasing card:", error);
        return { success: false, message: error.message || "فشلت عملية الشراء" };
    }
}

// --- Card Package Actions ---

export async function getCardPackages(): Promise<CardPackage[]> {
    try {
        const querySnapshot = await getDocs(collection(db, 'card_packages'));
        return querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as CardPackage));
    } catch (error) {
...
export async function addCardPackage(pkg: Omit<CardPackage, 'id'>): Promise<CardPackage | null> {
    try {
        const docRef = await addDoc(collection(db, 'card_packages'), pkg);
        return { id: docRef.id, ...pkg };
    } catch (error) {
        console.error("Error adding card package:", error);
        return null;
    }
}

export async function updateCardPackage(id: string, data: Partial<CardPackage>): Promise<boolean> {
    try {
        await updateDoc(doc(db, 'card_packages', id), data);
        return true;
    } catch (error) {
        console.error("Error updating card package:", error);
        return false;
    }
}

export async function deleteCardPackage(id: string): Promise<boolean> {
    try {
        await deleteDoc(doc(db, 'card_packages', id));
        return true;
    } catch (error) {
        console.error("Error deleting card package:", error);
        return false;
    }
}
